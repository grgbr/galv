...
static
enum galv_sess_head_multi
galv_sess_head_multi(const struct galv_sess_head * __restrict head)
{
	galv_assert_intern(head);

	return (head->flags >> GALV_SESS_HEAD_MULTI_FLAG_BIT) &
	       GALV_SESS_HEAD_MULTI_FLAG_MASK;
}

static
enum galv_sess_head_type
galv_sess_head_type(const struct galv_sess_head * __restrict head)
{
	galv_assert_intern(head);

	return (head->flags >> GALV_SESS_HEAD_TYPE_FLAG_BIT) &
	       GALV_SESS_HEAD_TYPE_FLAG_MASK;
}

static
unsigned int
galv_sess_head_xchg(const struct galv_sess_head * __restrict head)
{
	galv_assert_intern(head);

	return (unsigned int)head->xchg;
}
static inline
size_t
...
#warning FIXME: do not convert from network byte order if unix socket !
	return (size_t)be16toh(head->size);
}
...
galv_sess_init_msg(struct galv_sess_msg * __restrict message)
{
	...
	message->busy = 0;
	message->multi = GALV_SESS_MSG_MULTI_NR;
	message->type = GALV_SESS_MSG_TYPE_NR;
	galv_sess_reset_sgmt(&message->sgmt);
